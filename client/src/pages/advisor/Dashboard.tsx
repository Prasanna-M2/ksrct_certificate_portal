...
export const AdvisorDashboard: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      console.error('Failed to fetch Advisor dashboard data:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  // Class Advisor receives requests after Mentor approval (stage ADVISOR_REVIEW)
  const pendingCerts = certificates.filter(
    (c) => c.currentStage === 'ADVISOR_REVIEW' || c.status === 'ADVISOR_REVIEW'
  );
  const pendingOds = odRequests.filter(
    (o) => o.currentStage === 'ADVISOR_REVIEW' || o.status === 'ADVISOR_REVIEW'
  );
  const recentApproved = [
    ...certificates.filter((c) => c.currentStage === 'HOD_REVIEW' || c.status === 'APPROVED'),
    ...odRequests.filter((o) => o.currentStage === 'HOD_REVIEW' || o.status === 'APPROVED'),
  ];
  const recentRejected = [
    ...certificates.filter((c) => c.status === 'REJECTED' && c.rejectedByRole === 'ADVISOR'),
    ...odRequests.filter((o) => o.status === 'REJECTED' && o.rejectedByRole === 'ADVISOR'),
  ];
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="bg-gradient-to-r from-slate-900 via-ksrct-navy to-slate-900 text-white p-6 sm:p-8 rounded-3xl shadow-xl border border-white/10 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-extrabold bg-blue-400/20 text-blue-300 border border-blue-400/30 mb-2">
            <ShieldCheck className="w-3.5 h-3.5" />
            <span>CLASS ADVISOR DASHBOARD — LEVEL 2 APPROVAL AUTHORITY</span>
          </div>
          <h1 className="text-2xl font-black text-white">Welcome, {user?.name}</h1>
          <p className="text-xs text-slate-300 font-medium mt-1">
            Reviewing requests approved by Faculty Mentors before forwarding to HOD for final sign-off.
          </p>
        </div>
        ...
        >
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs uppercase font-bold tracking-wider mt-0.5">Approved to HOD</p>
            </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-slate-100 pb-3 gap-3">
          <div>
            <h2 className="text-base font-bold text-slate-900">
              {activeTab === 'PENDING_CERTS'
                ? 'Advisor Certificate Approvals Queue (Mentor Approved)'
                : activeTab === 'PENDING_OD'
                ? 'Advisor OD Applications Queue (Mentor Approved)'
                : activeTab === 'RECENT_APPROVED'
                ? 'Recently Advisor Approved Requests'
                : 'Recently Advisor Rejected Requests'}
            </h2>
            <p className="text-xs text-slate-500">
              Only requests approved by Faculty Mentors appear in this queue.
            </p>
          </div>
          ...
          <table className="w-full text-left text-xs border-collapse">
            ...
            <tbody className="divide-y divide-slate-100 font-medium">
              {loading ? (
                ...
                <tr>
                  <td colSpan={7} className="py-10 text-center text-slate-400">
                    No pending certificates awaiting Advisor approval.
                  </td>
                </tr>
              ) : activeTab === 'PENDING_OD' && pendingOds.length === 0 ? (
                <tr>
                  <td colSpan={7} className="py-10 text-center text-slate-400">
                    No pending OD requests awaiting Advisor approval.
                  </td>
                </tr>
              ) : (
                (activeTab === 'PENDING_CERTS'
